...
public:
    ...
    Node* get_bucket(int index) const {
        Node* current = table[index];
        return current;
    }
    ...
    bool sameMultiset(const HashTable& otra) const
    {
        size_t total1 = 0;
        size_t total2 = 0;
        ...
        for (size_t i = 0; i < capacity; i++)
        {
            ...
            while (current != nullptr)
            {
                total1 += current->value;

                const Value* frecuenciaOtra = otra.search(current->key);

                if (frecuenciaOtra == nullptr)
                    return false;

                if (current->value != *frecuenciaOtra)
                    return false;
        ...
        for (size_t i = 0; i < otra.capacity; i++)
        {
            Node* current = otra.table[i];

            while (current != nullptr)
            {
                total2 += current->value;
                current = current->next;
            }
        }
...
int main(){
    ...
    HashTable <int,int,DivisionHash<int>> ht2(5,3,0.5);
